...
async function fetchActivities() {
  ...
  error.value = null;
  try {
    const response = await axios.get(import.meta.env.VITE_API_BASE + `/api/events/myEvents.php`);
...
async function handleCancelActivity(activityId) {
  try {
     const response = await axios.post(import.meta.env.VITE_API_BASE + `/api/events/cancelEvent.php`, { id: activityId });
...
async function handleToggleFavorite(activityId) {
  ...
  try {
    const apiUrl = import.meta.env.VITE_API_BASE + `/api/events/eventToggleFavorite.php`;
