...
// --- 計算圖片來源---
const VITE_API_BASE_URL = import.meta.env.VITE_API_BASE;
...
function openModal() { isModalVisible.value = true }
...
<template>
    <!-- Main Card Container-->
    <div
        class="flex flex-col mx-5 md:mx-auto mt-5 bg-white rounded-lg overflow-hidden shadow-[5px_5px_10px_rgba(255,255,255,0.6)]">
        ...
        <div class="flex flex-col md:flex-row items-center gap-6 p-4">
            ...
            <div class="flex-grow w-full">
                ...
                <p class="mt-2 text-lg text-gray-400">{{ categoryName }}</p>
            ...
            <div class="flex flex-shrink-0 gap-4">
                ...
                <div class="flex flex-col items-center justify-center bg-[#F2994A] px-3 py-3 text-white w-36 h-36">
                    ...
                    <span class="text-[20px] font-bold md:text-[24px]">{{ quantity }} 張</span>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-gray-200 p-4 flex justify-end items-center gap-4">
            ...
            <div v-if="status === 'past'" class="flex justify-end items-center gap-4">
                <button @click="goToEventReview(props.id)"
            ...
            <div v-if="status === 'upcoming'" class="flex items-center justify-end gap-4">
                ...
                <!-- 查看票券按鈕 -->
                <button @click="goToTicketDetail(id)"
                    class="bg-[#F2994A] hover:bg-orange-500 text-white font-bold py-2 px-6 rounded-lg">
                    查看票券詳情
                </button>
            </div>
            <!-- 情況四：活動已取消 -->
        ...
        <div v-if="isModalVisible"
            ...
            <div class="bg-white rounded-lg shadow-xl p-6 w-full max-w-sm">
                ...
                <p class="mt-4 text-base text-gray-500">
                    取消後將無法再次參加本次活動，因您已過報名截止日取消，本次收費將無法退款，謝謝您。
                </p>
                <div class="mt-6 flex gap-4">
                    <button @click="confirmCancellation"
                        class="flex-1 bg-[#D96570] hover:bg-red-700 text-white font-bold py-2 px-4 rounded-md transition-colors">
                        確認退票
                    </button>
                    <button @click="closeModal"
                        class="flex-1 bg-white hover:bg-gray-200 text-gray-700 border border-gray-400 font-bold py-2 px-4 rounded-md transition-colors">
                        返回
                    </button>
                </div>
            </div>
